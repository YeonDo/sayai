...
public class FantasyGameDetailsTimezoneTest {
    ...
    public void getGameDetails_ShouldReturnZonedNextPickDeadline() throws Exception {
        ...
        mockMvc.perform(get("/apis/v1/fantasy/games/" + game.getSeq() + "/details"))
                ...
                // Expecting ISO 8601 with timezone offset (Z, +09:00, etc.)
                // Since we use ZoneId.systemDefault(), the output depends on the test environment.
                // We just verify that it has SOME timezone offset indicator (Z or +HH:mm or -HH:mm)
                .andExpect(jsonPath("$.nextPickDeadline", matchesPattern(".*(Z|[+-]\\d{2}:\\d{2})$")));
    }
}
